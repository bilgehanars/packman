...
export default class PackManPage extends ExtensionPage {
  oninit(vnode) {
    ...
    .then(result => { // ok işareti ile tanımlanan fonksiyon
      console.log(result);
  ...
  content() {
    return (
      <div className={"container"}>
        <Alert> While the page is loading, PackMan will automatically print the applications that are required by composer for flarum. It may take a minute. </Alert>
        <form onsubmit={this.onsubmit.bind(this)}>
          ...
          Package Name (Example: Bilgehanars/PackMan)<br />
        ...
        {m('div', [
  this.output !== '' && m('div.PackManPage-output', m('pre', this.output))
])
        }
      </div>
    );
